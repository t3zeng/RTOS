...
bool is_violated( pair_t pair ) {
	...
	if ( rslt ) {
		if(DO_PRINT) printf("The %d Byte block starts at %d overlaps with the %d Byte block starts at %d\n", pair.first.len, pair.first.ptr, pair.second.len, pair.second.ptr);
	}
	return rslt;
}
...
uint32_t log_2( uint32_t n ) {
	...
	if ( n <= 0 ) {
		if(DO_PRINT) printf( "log_2(0) is wrong\n" );
	}
...
bool test_max_alc( void ) {
	...
	if(DO_PRINT) printf("blk_sz = %d", blk_sz);
...
bool test_static_alc_free_violation( void ) {
	...
	if ( blks_sz == 0 ) {
		if(DO_PRINT) printf( "Failure on allocating any memory block. The memory access violation is irrelevant.\n" );
		return false;
	}
...
bool test_rndm_alc_free( void ) {
	...
	for ( i = 0; i < RNDM_TESTS; ++i ) {
		...
		if ( blk.ptr != 0 ) {
			...
			++blks_sz;
			alc_rec++;
			if(DO_PRINT) printf( "%i)The allocated %d Byte block starts from %d \n", ++line, blk.len, blk.ptr );
		}
	}
	...
	for ( i = 0; i < RNDM_TESTS >> 1 ; ++i ) {
		if ( (rand() % 2) && (blks_sz > 0) ) {
			...
			if(DO_PRINT) printf("%i)The %d Byte block starting from %d is free1\n", ++line, blks[tbf].len, blks[tbf].ptr);
			--blks_sz;
	...
	for ( i = blks_sz - 1; i >= 0; --i ) {
		...
		--blks_sz;
		if(DO_PRINT) printf("%i)The %d Byte block starting from %d is free2\n", ++line, blks[i].len, blks[i].ptr);
	}

	// All allocated memories have to be freed now.

	if(DO_PRINT) printf("%d random blocks are allocated and freed without any violation.\n", alc_rec);
